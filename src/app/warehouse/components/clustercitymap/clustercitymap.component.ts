...
@Component({
  selector: 'app-clustercitymap',
  templateUrl: './clustercitymap.component.html',
  styleUrls: ['./clustercitymap.component.scss']
})
export class ClustercitymapComponent implements OnInit {
  ...
  apiresponse: any;
  whStorelist:any;
  duplicateclusterWiseCityWise:any;
  ...
  constructor(private messageService: MessageService, private router: Router,
    ...
    this.apiresponse = {};
    this.whStorelist = [
      { label: "B2B", value: false },
      { label: "B2C", value: true }
    ];
    // this.IsStoreInputt=this.whStorelist.filter(x=>x.value==false && x.label=='B2B')
    // this.IsStoreInputt = this.whStorelist.find(option => option.label === 'B2B').value;
    this.IsStoreInputt = this.whStorelist[0];  // Sets "B2B" as the default selection
  ...
  ngOnInit() {
    ...
    this.clusterservice.GetMap(this.cityid).subscribe(result => {
      ...
      this.duplicateclusterWiseCityWise = JSON.parse(JSON.stringify(result));
  ...
  IsStoreInputt:any;
  duplicates:any;
  OnStoreInput(IsStoreInput){
    // alert("dfgdfgdf");
    console.log(this.duplicateclusterWiseCityWise)
    console.log(this.clusterWiseCityWise)
    console.log(IsStoreInput);
    
    debugger
    if(IsStoreInput == true){
      this.clusterWiseCityWise.clusters.forEach(x => {
       if(x.IsStore == false){
        let index = this.clusterWiseCityWise.clusters.indexOf(x);
        this.clusterWiseCityWise.clusters.splice(index, 1);
       }
      });
      console.log(this.clusterWiseCityWise)
      console.log(this.duplicateclusterWiseCityWise)
      ...
      const duplicate = JSON.parse(JSON.stringify( this.duplicateclusterWiseCityWise));
      this.clusterWiseCityWise =duplicate; // this.duplicateclusterWiseCityWise;
      this.lat = this.clusterWiseCityWise.city.CityLatitude;
      this.lng = this.clusterWiseCityWise.city.CityLongitude;
      this.generatePolygons();
    }
