...
@Component({
  selector: "app-vendor-payment-automation",
  templateUrl: "./vendor-payment-automation.component.html",
  styleUrls: ["./vendor-payment-automation.component.scss"],
  animations: [
...
export class VendorPaymentAutomationComponent implements OnInit {
  ...
  search() {
    console.log(this.payload, this.date);
    ...
    if (!this.warehouseids || this.warehouseids.length < 1 || !this.date) {
      ...
      this.isLoading = true;
      // debugger
      this.vendorPaymentService.GetRegionalListByAllWh({ Warehouseids: warehouses, ForDate: this.payload.StartDate, Keyword:this.keyword }).subscribe(
  ...
  RegionalTranspoterPaymentDetailList(rowdata: any) {
    ...
    let filterWarehouse = this.warehouseList.filter(x => { return x.label.includes('( ' + rowdata.WarehouseName + ' )') })
    let wid =0;
    if (filterWarehouse && filterWarehouse.length > 0) {
      wid = filterWarehouse[0].value;
    }
  ...
  ExportPaymentFile() {
    if (this.tellyIdsColltn.length > 0) {
      ...
      this.vendorPaymentService.ExportPaymentFileV3(paytload).subscribe((x: any) => {
        console.log("ExportPaymentFileV3", x);
        if (x) {
          let string = this.baseURL + x;
          window.open(string);
        } else {
      ...
      alert("Please select vendors before generating the report for payment")
    }
  ...
  ExportRegiobalFile() {
    let warehouses = this.warehouseids.map(x => x.value);
    let firstDay = moment(new Date(this.date.getFullYear(), this.date.getMonth(), 1)).format("YYYY-MM-DD");
    this.vendorPaymentService.RegionalExportV2({ Warehouseids: warehouses, ForDate: firstDay }).subscribe((x: any) => {
