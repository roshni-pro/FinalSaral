...
@Component({
  ...
  styleUrls: ['./sales-person-report.component.scss']
})
...
this.waypoints = [];
  ...
  ngOnInit() {
    ...
    this.warehouseService.GetAllWarehouse().subscribe(result => {
      ...
      // this.salesAppCounterService.getSalesInitialPoint(this.selectedWarehouseId).subscribe(pointList => {
      ...
      //   console.log('allMakers',this.allMarkers);
  ...
  searchSalesTarget(salesMapForm)
  {
    
    if (salesMapForm.form.status == "VALID") {
  ...
  onChangeFollowTarger() {
    if (this.followMarker != '') {
      ...
      this.fromDate = this.data.DOI;
      let frmDate = moment(this.fromDate).format('YYYY-MM-DD');
      this.salesAppCounterService.getSalesPersonPointsNew(this.followMarker.id, frmDate, frmDate).subscribe(result => {
        console.log('result is : ', result);
        this.wayPoint = [];
        if (result != null && result.length > 0) {
          result.forEach(item => {
            this.wayPoint.push({
              location: { lat: Number(item.lat), lng: Number(item.Long), date : item.Date },
              stopover: false,
  ...
  //this.origin = { lat: 22.7272704, lng: 75.8815212 };
  // this.destination = { lat: 22.7274823, lng: 75.8831879 };
  // this.waypoints = [
  //   { location: { lat: 22.7272668, lng: 75.8815225 } },
  //   { location: { lat: 22.7272617, lng: 75.8815284 } },
  //   { location: { lat: 22.7273441, lng: 75.8816848 } },
  //   { location: { lat: 22.7274272, lng: 75.8814455 } },
  //   { location: { lat: 22.7276014, lng: 75.8820204 } }
  // ];

          console.log('this.wayPoint: ', JSON.stringify(this.wayPoint));
  ...
  filterSalesPerson(event)
  {
    this.salesAppCounterService.getSalesPersonDetails(event.query).subscribe(x=>
      {
        this.salesList = x;
        console.log('auto:',this.allMarkers)
      })
  }

  onSelectsales(event)
  {
    this.followMarker = event;
  }
  ...
  markerClick(infoWindow, $event: MouseEvent) {
    infoWindow.open();
  }
